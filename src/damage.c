...
.defenseCount: each attack is divided by defenseCount (default is by 1, in case
of defense is by 2). .weaponDamage: a weapon wears out with use, if it reaches a
value of 3, it is thrown away because unusable. The player has an impact of 1,
the aggressor of 2 and the weapons from 3 and above.

in describeAttack, the defense is now taken into account and a health count is
shown. in dealDamage, if you do not have weapons, the weaponDescription is
chosen randomly. in dealDamage, the damage formula has been changed in a more
aggressive way. in dealDamage, it's now described if the opponent miss a hit
(when damage = 0). in getOptimalWeapon, when a weapon loses is power, the
opponent trows it away.
*/
    ...
    if (weapon->health == 0) {
        ...
        if (weapon->impact == 3) {
            if (weapon->location == player)
                player->weapon--;
            printf("The %s breaks from the impact and falls on the ground, "
                   "unusable\n",
                   weapon->explanation);
...
static void describeAlreadyDead(OBJECT* attacker, OBJECT* victim) {
    if (victim == player) {
        printf("Even though you're dead, the %s continue to attack you with "
               "fury.\n",
               attacker->explanation);
    } else {
