...
.executeDefense: increseas defenseCount from 1 to 2, halving the gravity of an
attack (check damage.c) .reachableObjectDefense: allows to use "yourself" as an
object (check reach.c)
*/
...
int executeAttack(void) {
    OBJECT* victim = *params[0] == '\0'
                         ? victimHere()
                         : reachableObject("who to attack", params[0]);
    if (victim != NULL) {
        OBJECT* weapon = *params[1] == '\0'
                             ? getOptimalWeapon(player)
...
int executeDefense(void) {
    OBJECT* victim =
        *params[0] == '\0'
            ? victimHereDefense()
            : reachableObjectDefense("who you want to protect yourself from",
                                     params[0]);
    if (victim != NULL) {
        dealDefense();
        return 1;
