...
obj->description is used to describe the whole scene and obj->explanation to
name individual objects (check object.txt) .player->weapon: the player can carry
only 2 weapons (check object.txt). .player->capacity: the player can carry a
limited amount of objects, based on capacity. .objects with a capacity of 100
have a special message on why they cannot be carried.
*/
...
int moveObject(OBJECT* obj, OBJECT* to) {
    if (obj == NULL) {
        ...
        printf(obj == keyForGarage &&
                       (to == closedDoorToGarage || to == lockedDoorToGarage)
                   ? "The key seems to fit the padlock.\n"
                   : "It doesn't seem to fit in.\n");
        return 1;
    } else if (obj->weight > to->capacity) {
        if (obj->weight == 100) {
            printf("You feel too weak, better not to waste energy carrying "
