...
.fireCountdown: when the burn command is executed, trigger is set at 100000 and
increases by 1 with every move of the player. (check executeBurn in
inventory2.c) .fireDestroy: each location on the map is destroyed by the
spreading fire, based on the trigger and their sector. Each object with
attribute burn=1 is moved to emptyRoom and disappears from the map. (check
object.txt) .fireEnd: the trigger shows various messages, based on its value. It
also acts as a countdown, and when trigger reaches 100020, the player dies.
*/
...
int fireEnd(void) {
    ...
    if (trigger == 100003) {
        printf("\033[0;31m");
        printf("\nDry grass and the rest of the gasoline help the fire spread "
        ...
        printf("\033[0;31m");
        printf("\nThe house is completely engulfed in flames and it's about to "
               "collapse on itself.\n");
        printf("\033[0m");
        ...
        printf("\033[0;31m");
        printf("\nThe fire has spread too fast for you to escape. You are "
               "caught in the flames and burned alive.\n");
        printf("\033[0m");
