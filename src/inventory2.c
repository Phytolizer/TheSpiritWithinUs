...
.executeKill: based on the health of the enemy, and whether or not the enemy is
an animal. .executeDrive: being the car the way to end the game in case of
victory, the drive command recalls the relative ending (check end.c)
.executeRead: to read objects with read attribute.
.executeBurn: depending on different conditions you can burn or not burn some
objects. In case you burn the corpse of your enemy, the function fireContdown is
recalled (check fire.c). .executeEat: to eat objects with power attribute. In
case power is negative, the player's health is reduced. After eating it, the
object is moved to emptyRoom and disappears from the map.
*/
...
int executeKill(void) {
    ...
    switch (getDistance(player, obj)) {
        ...
        case distUnknownObject:
            ...
            break;
        default:
            if (player->location == man->location &&
                (man->health > 0 && man->health <= 5)) {
                man->health = 0;
                printf("With one quick move you put your hands on the old "
                       "janitor's neck and start squeezing, first with fear "
                       "and then with more rage.\n");
                printf("His eyes start to cry and you can feel his throat give "
                       "out. He tries to hit you but his fists are weak.\nYou "
                       "just killed a man.\n");
            } else if (player->location == man->location && man->health > 5) {
                printf("You try to choke the old janitor but he is too strong "
                ...
                printf("The %s is too fast for your to catch and chocke. You "
                       "attack but miss.\n",
                       obj->explanation);
                obj->trust = -1;
    ...
    switch (getDistance(player, obj)) {
        ...
        default:
            if (gas->location == player && matches->location == player) {
                ...
                } else if (player->location == man->location &&
                           man->health == 0) {
                    printf("You set fire to everything, you don't have much "
                           "time to escape before the fire spreads.\n");
                    man->health = -1;
                    fireCountdown();
                } else if (player->location == obj->location &&
                           obj->health == 0) {
                    printf("You better use the little gas you have to burn the "
                           "real evidence.\n");
                }
            } else {
        ...
        default:
            if (obj->power != 0 && obj->location == player) {
                ...
                if (obj->power > 0) {
                    ...
                    printf("The food you just ate makes you feel bad, the %s "
                           "was toxic.\n",
                           obj->explanation);
                }
                printf("\033[0;34m");
