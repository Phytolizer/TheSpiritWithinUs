...
.fox: if the fox is alive and not in the same location as the player, it will
keep moving.
*/
...
void fox(void) {
    if (oldFox->location != player->location && oldFox->health > 0 &&
        oldFoxDead->location == emptyRoom) {
        if (foxCount > 12)
            foxCount = 0;
        foxLocation();
    }
